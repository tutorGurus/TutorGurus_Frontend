      ...
      <v-container>
        <v-row>
          <v-col cols="6">
            <v-row>
              <h3>每週固定課表</h3>
              <v-data-table :headers="headers" :items="result" disable-sort disable-pagination hide-default-footer>
                ...
                <v-card-text>
                  <div v-for="(day, name) in weeklySchedule" :key="day">
                    <v-row>
                      ...
                      <v-col cols="3">
                        <v-select v-model="day.startTime" :items="hours_list" :disabled="!day.open" label="Start" />
                      </v-col>
                      <v-col cols="3">
                        <v-select v-model="day.endTime" :items="hours_list" :disabled="!day.open" label="End" />
                      </v-col>
                    </v-row>
                  </div>
                </v-card-text>
