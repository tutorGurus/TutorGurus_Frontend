...
<template>
  <v-card>
    <v-toolbar class="text-white" color="orange-darken-1">
      <v-toolbar-title>{{ isRegister === false ? '登入' : '註冊' }}</v-toolbar-title>
      <template v-slot:append>
        <p v-if="!isRegister">
          沒有帳號，<span class="text-decoration-underline cursor-point" @click="toRegister"
            >快速註冊</span
          >
        </p>
        <p v-else>
          已經註冊，<span class="text-decoration-underline cursor-point" @click="isRegister = false"
            >登入</span
          >
        </p>
      </template>
